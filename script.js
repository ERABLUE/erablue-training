...
async function initData() {
    try {
        ...
        masterLib = l; masterHelper = h; masterRank = r;
...
function renderRankList() {
    ...
    document.getElementById('rankList').innerHTML = sorted.map((item, i) => `
...
// LOGIKA HELPER (DIPERBAIKI AGAR FLEKSIBEL)
function cariError() {
    const sn = document.getElementById("helperSN").value.toLowerCase().trim();
    const err = document.getElementById("helperError").value.toLowerCase().trim();
    const container = document.getElementById("helperResult");

    const filtered = masterHelper.filter(item => {
        const matchSN = sn === "" || item.serialNumber.includes(sn);
        const matchErr = err === "" || item.kodeError.includes(err);
        return matchSN && matchErr; // Jika salah satu kosong, tetap dianggap cocok (True)
    });

    if (filtered.length === 0) {
        container.innerHTML = "<p style='text-align:center; padding:20px;'>❌ Tidak ditemukan.</p>";
        return;
    }

    container.innerHTML = filtered.map(item => `
        <div class="result-item" style="border-left: 6px solid #d32f2f;">
            <small>${item.merk} - SN: ${item.serialNumber.toUpperCase()}</small><br>
            <strong style="color:#d32f2f; font-size:1.1rem;">Error: ${item.kodeError.toUpperCase()}</strong><br>
            <p style="font-weight:bold; margin-top:5px;">Arti: ${item.artiKode}</p>
            <div style="background:#fff9f9; padding:10px; border-radius:8px; margin-top:8px; border:1px solid #ffdada;">
                <p style="font-size:0.9rem;">${item.solusi}</p>
            </div>
        </div>
    `).join('');
}

// FUNGSI LAIN (TETAP SESUAI ASLI)
    ...
    document.getElementById("libResult").innerHTML = filtered.map(item => `
        <div class="result-item">
            ...
            <div style="display:flex; gap:8px; margin-top:10px;">
                <a href="${item.linkPPT}" style="background:#0d47a1; flex:1; color:white; padding:10px; border-radius:8px; text-align:center; text-decoration:none;" target="_blank">PPT</a>
                <a href="${item.linkBook}" style="background:#e65100; flex:1; color:white; padding:10px; border-radius:8px; text-align:center; text-decoration:none;" target="_blank">BOOK</a>
            </div>
        </div>
    `).join('');
}
...
// ==========================================
// FUNGSI KHUSUS TAB USER BARU (DITAMBAHKAN TANPA MENYENTUH YANG LAIN)
// ==========================================
...
async function handleSignup() {
    ...
    document.querySelector('#signupView .btn-action').innerText = "Memproses...";
    ...
    try {
        ...
        document.querySelector('#signupView .btn-action').innerText = "Sign In";
    }
}
...
async function handleLogin() {
    ...
    document.querySelector('#loginView .btn-action').innerText = "Memproses...";
    ...
    try {
        ...
        document.querySelector('#loginView .btn-action').innerText = "Login";
    }
}
...
function showProfile(data) {
    ...
    if (certBox.innerHTML === "") {
        certBox.innerHTML = "<p style='font-size:0.8rem; color:#888;'>Belum ada sertifikat.</p>";
    }
}
